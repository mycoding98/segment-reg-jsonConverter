 ...
 */
export async function processSegmentationFile(filePath, outputDir) {
 ...
 * @param outputDir - Directory to save the JSON outputs.
 */
async function processExcelFile(filePath, outputDir) {
 ...
 */
async function generateSegmentationJson(rows, outputDir, segmentName) {
    ...
    for (const [brand, brandRows] of Object.entries(groupedRows)) {
        ...
        for (const [preferenceType, mapping] of Object.entries(fieldMapping)) {
            ...
            const rowChunks = filteredRows.length > 200 ? splitArrayIntoChunks(filteredRows, Math.ceil(filteredRows.length / 2)) : [filteredRows];
